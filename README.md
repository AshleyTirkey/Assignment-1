...
## Application Domain: Public Transit Authority (RBAC)
...
1. **Roles:** Which roles are governed by this policy?
  *(Values: `TransitDirector`, `TrainOperator`, `Commuter`)*
...
2. **Action:** Operation to check.
  *(Values: `Reroute`, `Drive`, `Board`)*
3. **Resources:** A list of resources this rule applies to.
  *(Values: `ControlCenter`, `SubwayTrains`, `TransitPasses`)*
...
## Example 1: The "PublicTransitAuthority" Policy
**The Scenario:** `TransitDirector` principals may perform `Reroute` on `ControlCenter`, but no other access is granted.

**The Declarative JSON Rule:**
```json
{
  "NAME": "PublicTransitAuthority",
  "ROLES": ["TransitDirector"],
  "RULES": [
    {
      "EFFECT": "Allow",
      "ACTION": "Reroute",
      "RESOURCES": ["ControlCenter"]
    }
  ]
}
```
...
**The Scenario:** `TrainOperator` and `Commuter` principals are in scope. They may perform `Drive` on `SubwayTrains`, but `Board` on `SubwayTrains` is explicitly denied.

**The Declarative JSON Rule:**
```json
{
  "NAME": "SecureAccess",
  "ROLES": ["TrainOperator", "Commuter"],
  "RULES": [
    {
      "EFFECT": "Allow",
      "ACTION": "Drive",
      "RESOURCES": ["SubwayTrains"]
    },
    {
      "EFFECT": "Deny",
      "ACTION": "Board",
...
*   **The Role Block (`rbac_role`):** Previous and next connections (type `rbac_role`), and a dropdown field `ROLE` with values `TransitDirector`, `TrainOperator`, `Commuter`.
*   **The Rule Block (`rbac_rule`):** Previous and next connections (type `rbac_rule`), a dropdown `EFFECT` (`Allow`/`Deny`), a dropdown `ACTION` (`Reroute`, `Drive`, `Board`), and a statement input `RESOURCES`.
*   **The Resource Block (`rbac_resource`):** Previous and next connections (type `rbac_resource`), and a dropdown field `RES` with values `ControlCenter`, `SubwayTrains`, `TransitPasses`.
...
*   **`rbac_role`:** Return the `ROLE` field value as a quoted string, e.g. `"TRANSITDIRECTOR"`.
*   **`rbac_resource`:** Return the `RES` field value as a quoted string, e.g. `"CONTROLCENTER"`.
